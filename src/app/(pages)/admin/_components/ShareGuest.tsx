...
export function ShareGuest({
  isOpen,
  onClose,
  selectedGuest,
}: Readonly<ShareGuestProps>) {
  const options = [
    {
      value: "formal",
      label: "Formal",
  ...
  const handleShare = () => {
    ...
    const phone = selectedGuest?.whatsapp
      ? `phone=${selectedGuest?.whatsapp}`
      : "";
    let url = ``;
    if (selectedGuest?.whatsapp === "628") {
      ...
      url = `https://api.whatsapp.com/send?${phone}&text=${encodeURIComponent(
        message
      )}`;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        ...
        className="text-cream bg-gray-900"
      >
        <DialogHeader>
        ...
        <div className="flex items-center space-x-2">
          <div className="grid flex-1 gap-2">
            ...
            <Combobox
              options={options}
              onChange={(value) =>
                setMessageStyle(value as "formal" | "santai" | "keluarga")
              }
        ...
        {selectedGuest && (
          <div className="mt-2 p-2 border border-gray-200 rounded-md bg-gray-700 text-sm relative">
            <h3 className="mb-1">📄 Preview Pesan WhatsApp</h3>
            <p>
              <strong>Untuk:</strong> {selectedGuest?.name}
            </p>
            <pre className="mt-2 whitespace-pre-wrap break-all border border-gray-200 rounded-md bg-gray-800 text-xs p-2">
              {generateWhatsAppMessage(selectedGuest)}
            </pre>
            <BtnCopy
              text={generateWhatsAppMessage(selectedGuest)}
